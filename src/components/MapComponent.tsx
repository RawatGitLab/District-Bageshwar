...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Bageshwar, Uttarakhand, India (Bageshwar: 29.96, 79.80)
      ...
      if (existingLayer) {
        ...
        try {
          existingLayer.setStyle((feature: any) => {
            ...
            return {
              color: isPoly ? "#ffffff" : layerConf.color,
  ...
  // Map Controls: Pan to Bageshwar bounds safely
  const handleZoomToDistrict = () => {
    ...
    // Fallback static center of Bageshwar
    map.setView([29.96, 79.80], 10, { animate: true });
  };
    ...
    <div className="relative flex-1 bg-slate-100 flex flex-col h-full min-w-0">
      ...
      <div className="absolute top-4 left-4 z-[1001] w-80 font-sans">
        <div className="relative flex items-center bg-white/95 dark:bg-slate-900/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-lg shadow-md transition-shadow duration-200 focus-within:shadow-lg focus-within:border-indigo-400">
          ...
          <input
            type="text"
            className="w-full text-xs pl-9 pr-8 py-2.5 bg-transparent rounded-lg text-slate-700 dark:text-slate-100 placeholder-slate-400 font-semibold focus:outline-none"
        ...
        {showMapSuggestions && filteredSearchFeatures.length > 0 && (
          <div className="absolute left-0 right-0 mt-1.5 bg-white/95 dark:bg-slate-900/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl overflow-hidden max-h-60 overflow-y-auto z-[1002] divide-y divide-slate-100 dark:divide-slate-800">
            {filteredSearchFeatures.map((feat) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-3.5 py-2.5 hover:bg-indigo-50/70 dark:hover:bg-slate-800/80 active:bg-indigo-100 dark:active:bg-slate-800 text-xs text-slate-700 dark:text-slate-200 font-medium transition-colors flex items-center justify-between gap-1 border-none bg-transparent cursor-pointer"
                >
                  <span className="flex items-center gap-2 truncate">
                    <MapPin className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                    <span className="truncate text-slate-800 dark:text-slate-100 font-semibold">{name}</span>
                  </span>
                  <span className="text-[9px] uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold px-1.5 py-0.5 rounded font-mono shrink-0">
                    {layerName.replace("USN-", "").replace("Almora-", "").replace("Bageshwar-", "").replace("-Boundary", "")}
                  </span>
                </button>
              );
            })}
