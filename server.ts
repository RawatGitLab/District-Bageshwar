...
function projectCoordinates(coordinates: any): any {
  ...
  if ((coordinates.length === 2 || coordinates.length === 3) && typeof coordinates[0] === "number" && typeof coordinates[1] === "number") {
    ...
    if (Math.abs(x) > 1000 || Math.abs(y) > 1000) {
      ...
      return [79.8025, 29.8789]; // safe fallback
    }
    return coordinates;
  }
...
if (!MONGODB_URI) {
  throw new Error("MONGODB_URI environment variable is required but not defined in the environment.");
}
